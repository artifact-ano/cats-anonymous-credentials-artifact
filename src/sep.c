...
static void store_u32(uint8_t *dst, uint32_t value) {
...
#endif
  uint64_t stateinc = ((uint64_t)load_u32(state)) + 1;
  store_u32(state, (uint32_t) stateinc);
  uint64_t carry = stateinc >> 32;
  for (i = 1; i < STATE_BYTES/4; i++)
  {
    stateinc = ((uint64_t)load_u32(&state[i*4])) + carry;
    store_u32(&state[i*4], (uint32_t) stateinc);
    carry = stateinc >> 32;
  }
