...
void random_init(void) {
  ...
  shake256_absorb_once(&prng_state, seed, sizeof(seed));
  prng_initialized = 1;
}
...
static void random_bytes(uint8_t * restrict data) {
  ...
  shake256_squeeze(data, 16, &prng_state);
}
#endif
...
static double algorithm_EA(uint64_t * n) {
  ...
  const unsigned long long int II = load_u64(data);
  const int j = __builtin_ctzll(II);
  const double G = c + j*ln2;

  const double U = load_u16(data + 8) / (UINT16_MAX*1.0);
  random_bytes(data);
  ...
  while(true) {
    for(unsigned i = 0; i < 4; i++) {
      (*n)++;
      const double U1 = load_u16(data + i*2) / (UINT16_MAX*1.0);
      const double U2 = load_u16(data + i*2 + 8) / (UINT16_MAX*1.0);
